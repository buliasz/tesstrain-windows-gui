...
> Tesseract language training GUI v6.0 for Tesseract 5+.
...
Please don't forget this fork is for GUI implementation developed mainly by one developer. The Tesseract and Tesstrain projects for which this GUI  is 
created are much larger projects. This one is just to facilitate usage of the great tool.

## Install

The GUI is now justa python file you can run with your Python installation.

### Requirements

All you need is version 5 or newer of Tesseract binaries (that include the training tools and matching leptonica bindings).

For Windows OS I do recommend downloading executables from the [Tesseract at UB Mannheim](https://github.com/UB-Mannheim/tesseract/wiki) repository. 
...
### Language data
Tesseract expects some configuration data (a file `radical-stroke.txt` and `*.unicharset` for all scripts). Those will be downloaded automatically by the GUI when needed from [this address](https://github.com/tesseract-ocr/langdata_lstm/raw/main/radical-stroke.txt) and placed in the configurable "Output data directory".
...
`data/MODEL_NAME-ground-truth`). Training will generate list of those files, and split into training and evaluation data, the ratio can be defined in the GUI.
